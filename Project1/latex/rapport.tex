...
\affiliation{University of Oslo, Oslo, Norway \\ Source code available at: \url{https://github.com/Caronthir/FYS3150}}
\date{\today}

\begin{abstract}
      Three numerical methods of solving the one-dimensional Poisson equation with Dirchelet
      boundary conditions are investigated. The equation is discretized and converted
      to a linear algebra problem, solved with the common LU-decomposition and the less
      common Thomas algorithm. The numerical errors and calculation speeds are discussed.
      The general and a specialized version of the Thomas algorithm is, as expected,
...
examined. Many important differential equations in science can be re-written to
the form
\begin{align*}
  \nabla^2 u = f
\end{align*}
by variable substitutions. It is therefore interesting to develop algorithms
and functions that can easily solve these types of equations, with small errors.

By discretizing the equation it can be written as
...
a more efficient (but conceptually equal) approach to solve the problem is possible: the Thomas algorithm.
First the general version of this algorithm is implemented, and then a
...
\begin{align*}
  b'_i = \frac{i+1}{i}
\end{align*}
and is found and proved by induction in section~\ref{sec:diagelm}.
...
after extensive attempts to locate the bug and reimplementing the method. Because
...
The computation timings are shown in table~\ref{tab:timing}. Solely in terms
of the timings, the Thomas methods are far superior to the LU decomposition,
...
faster, for much larger matrices and with remarkably smaller errors. The Thomas algorithm is essentialy
