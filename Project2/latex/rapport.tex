...
larger matrices, and numerical algorithms are needed to solve the problems. In
this paper, the direct Jacobi rotation and iterative Lanczos algorithms
...
A detailed description of Jacobi's rotation algorithm is available at REFERANSE?.
The general idea is rather straightforward, and is based on diagonalizing a symmetric
matrix $A$ through a series of orthogonal similarity transformations on the form
\begin{align*}
  T = S^T A S
\end{align*}
where $S$ is a Givens rotation matrix. The rotation angle is conveniently chosen to
systematically eliminate off-diagonal elements in $A$, until the largest
offdiagonal element is below some threshhold $\epsilon$. Since $\epsilon$ is
typically set to a small value, the offdiagonal elements are small and
the approximate eigenvalues can be extracted from the matrix diagonal.
